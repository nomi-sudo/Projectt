...
export default function SubcategoryPage() {
  ...
  const { products: filteredProducts, loading: prodLoading } = useProducts(
    slug as string,
    (subslug as string).replace(/-/g, " ")
  );
  ...
  if (!category || !subcategory) return <div className="container mx-auto px-4 py-12">Category or Subcategory not found</div>;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            {prodLoading ? "Loading..." : `Showing ${filteredProducts.length} results in ${category.name}`}
          </p>
        </div>
